...
public class Methods {
    ...
    public static void simulation (Stack<Content> contentStack,Queue<MovieCritic> criticMovieQueue,Queue<GameCritic> criticGameQueue){
                ...
                for(int l = 0;l < contentStack.size(); l++){
                    ...
                    if(theContent.getContentNumber()==0 && theContent.getArrivalDay()<=i && theMovieCritic.getLimit()>0){
                        ...
                        if (a == 0 && !evaluateGameArrayList.isEmpty()) {
                            sortEvaluateList(evaluateGameArrayList);
        ...
        movieSortByNameList(movieArrayList);
        System.out.println("Ratings:");
        for (Movie a : movieArrayList){
            System.out.println(a.getName()+" "+a.getYear()+" "+a.getRate());
        }
        gameSortByNameList(gameArrayList);
        for (Game a : gameArrayList){
            System.out.println(a.getName()+" "+a.getRate());
        }
    }
    public static ArrayList<GameCritic> sortEvaluateList(ArrayList<GameCritic> gameCriticArrayList){ // Method for sorting Critics according to their work time (lower to higher).
      for (int i = 0;i< gameCriticArrayList.size();i++) {
          if (i > 0) {
              if (gameCriticArrayList.get(i-1).getWorkTime()> gameCriticArrayList.get(i).getWorkTime()) {
                  ...
                  return sortEvaluateList(gameCriticArrayList);
              }
          }
      }
      return gameCriticArrayList;
    }
    public static ArrayList<Movie> movieSortByNameList (ArrayList<Movie> contentArrayList){
        for (int i = 1; i< contentArrayList.size();i++){
            Movie tempContent = contentArrayList.get(i);
            int index = i;
            while (index>0 && tempContent.getName().compareTo(contentArrayList.get(index-1).getName())<0){
                contentArrayList.set(index,contentArrayList.get(index-1));
                index--;
            }
            contentArrayList.set(index,tempContent);
        }
        return contentArrayList;
    }
    public static ArrayList<Game> gameSortByNameList (ArrayList<Game> contentArrayList){
        for (int i = 1; i< contentArrayList.size();i++){
            Game tempContent = contentArrayList.get(i);
            int index = i;
            while (index>0 && tempContent.getName().compareTo(contentArrayList.get(index-1).getName())<0){
                contentArrayList.set(index,contentArrayList.get(index-1));
                index--;
